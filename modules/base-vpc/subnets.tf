...
resource "aws_subnet" "public" {
  ...
  cidr_block = "${cidrsubnet(local.public_subnets_block, ceil(pow(var.region-az-count-mapping[var.region],0.5)), count.index - 1)}"
...
resource "aws_subnet" "private" {
  count      = "${var.region-az-count-mapping[var.region]}"
  vpc_id     = "${aws_vpc.this.id}"
  cidr_block = "${cidrsubnet(local.private_subnets_block, ceil(pow(var.region-az-count-mapping[var.region],0.5)), count.index - 1)}"
...
resource "aws_subnet" "fortknox" {
  count      = "${var.region-az-count-mapping[var.region]}"
  vpc_id     = "${aws_vpc.this.id}"
  cidr_block = "${cidrsubnet(local.fortknox_subnets_block, ceil(pow(var.region-az-count-mapping[var.region],0.5)), count.index - 1)}"
